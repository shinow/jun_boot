...
import com.project.system.domain.SysRoleDept;
import com.project.system.domain.SysRoleMenu;
import com.project.system.mapper.SysRoleDeptMapper;
...
public class SysRoleServiceImpl implements ISysRoleService {
    ...
    public int deleteRoleByIds(String ids) throws Exception {
        ...
        for (Long roleId : roleIds) {
            ...
            if(roleId == Global.ROLE_ID_DEFAULT){
                throw new Exception(StringUtils.format("{}为系统内置角色不能删除！", role.getRoleName()));
            }
            if (countUserRoleByRoleId(roleId) > 0) {
                throw new Exception(StringUtils.format("{}已分配,不能删除", role.getRoleName()));
            }
        }
    ...
    public int insertRole(SysRole role) {
        // 新增角色信息
        role.setRoleId(TimeUuidUtil.get16UUID());
    ...
    /**
     * 根据用户id获取该用户所有的角色中配置数据权限时所选的部门的jbdm
     *
     * @param userId
     * @return
     */
    @Override
    public List<String> queryDataScopeJbdmByUserId(Long userId) {
        return roleDeptMapper.selectDataScopeJbdmByUserId(userId);
    }
}
