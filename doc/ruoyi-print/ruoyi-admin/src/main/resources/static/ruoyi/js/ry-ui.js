...
(function ($) {
    $.extend({
        ...
        operate: {
			...
			print: function(id,tempId,orderType){
				table.set();
				if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
					var row = $("#" + table.options.id).bootstrapTreeTable('getSelections')[0];
					if ($.common.isEmpty(row)) {
					...
					var url = table.options.printUrl;
						url = url.replace("{id}", row[table.options.uniqueId]);
						url = url.replace("{tempId}", tempId);
						url = url.replace("{orderType}", orderType);
					$.modal.openFull("打印", url);
				} else {
					$.modal.openFull("打印", $.operate.printUrl(id,tempId, orderType));
				}
			},
			//打印访问页面
			printUrl: function (id, tempId, orderType) {
				var url = "/404.html";
				if ($.common.isNotEmpty(id)) {
					url = table.options.printUrl;
				} else {
					var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
					if (id.length == 0) {
						$.modal.alertWarning("请选择一条记录");
