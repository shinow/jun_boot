...
public class ServletUtils
{
     ...
     * @param rememberMe
     */
    public static void setLoginCookieFront(String username, String password, Boolean rememberMe){
       /* HttpServletResponse response= ServletUtils.getResponse();
        response.setHeader("Access-Control-Allow-Credentials","true");
        CookieUtils.setCookie(response, Constants.FRONT_COOKIE_USERNAME, username, Constants.SECONDS_7_DAYS);
        if (rememberMe) {//记住密码
            CookieUtils.setCookie(response, Constants.FRONT_COOKIE_PASSWORD, password, Constants.SECONDS_7_DAYS);
            CookieUtils.setCookie(response, Constants.FRONT_COOKIE_REMEMBERME, "1", Constants.SECONDS_7_DAYS);
        } else {
            CookieUtils.setCookie(response, Constants.FRONT_COOKIE_PASSWORD, password,0);//0表示清除Cookie
            CookieUtils.setCookie(response, Constants.FRONT_COOKIE_REMEMBERME, "0", Constants.SECONDS_7_DAYS);
        }*/
    }
