...
public class ExcelImportServiceImpl implements IExcelImportService {
    ...
    public ImportExcelResult saveData(List<Object> list, String importType) {
        ...
        for(int i=0;i<list.size();i++){
            switch (importType){
                case "1":
                    result.add(saveData((SysUser)list.get(i),i+1,importExcelResult));
                    break;
                case "2":
     ...
     * @return
     */
    public Map<String, String> saveData(SysUser user, int rowNum, ImportExcelResult importExcelResult) {
    ...
    public Map<String, String> saveData(SysDept dept, int rowNum, ImportExcelResult importExcelResult) {
        ...
        if(tempDept==null){
            ...
            int count = deptService.checkDeptCanAdd(tempDept.getDeptId());
            if(count==2){
                deptService.adjustDeptJbdm(tempDept.getDeptId());
            }else if(count==3){
                msg.append("上级部门（").append(dept.getParentName()).append("）的直接下级数量达到900上限！");
            }
        }
        if(!StringUtils.isEmpty(msg.toString())){
    ...
    private SysDept getDeptByName(String deptName){
        ...
        List<SysDept> depts = deptService.queryDeptList(dept,null);
