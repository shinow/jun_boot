...
public class ExcelUtil<T> {
        ...
        if (rows > 0) {
            ...
            for (int col = 0; col < allFields.length; col++) {
                ...
                if (field.isAnnotationPresent(ExcelImport.class)) {
                    ...
                    fieldsMap.put(excelImport.colNum(), field);
                    serialNum++;
                }
            }
     ...
     * @return
     */
    public AjaxResult exportExcelModel(String sheetName) {
        Field[] allFields = clazz.getDeclaredFields();
        Map<Integer, Field> heads = new HashMap<Integer, Field>();
        int totalCol = 0;
        // 得到所有field并存放到一个list中.
        ExcelImport attr = null;
        for (Field field : allFields) {
            if (field.isAnnotationPresent(ExcelImport.class)) {
                attr = field.getAnnotation(ExcelImport.class);
                heads.put(attr.colNum(), field);
                totalCol++;
            }
        }
        ...
        try {
            ...
            for (int i = 0; i < totalCol; i++) {
                ...
                writeHeadCell(workbook, sheet, cell, i, attr.colName(), attr.prompt());
            }
            String filename = encodingFilename(sheetName);
            out = new BufferedOutputStream(new FileOutputStream(getAbsoluteFile(filename)));
            workbook.write(out);
    ...
    private void writeHeadCell(HSSFWorkbook workbook, HSSFSheet sheet, HSSFCell cell, int col, String value, String prompt) {
        // 设置列中写入内容为String类型
        cell.setCellType(CellType.STRING);
        HSSFCellStyle cellStyle = workbook.createCellStyle();
        cellStyle.setAlignment(HorizontalAlignment.CENTER);
        cellStyle.setVerticalAlignment(VerticalAlignment.CENTER);
        if (value.indexOf("注：") >= 0) {
        ...
        if (StringUtils.isNotEmpty(prompt)) {
            // 这里默认设了2-101列提示.
            setHSSFPrompt(sheet, "" , prompt, 1, 100, col, col);
        }
    }
        ...
        try {
            ...
            for (int index = 0; index <= sheetNo; index++) {
                ...
                for (int i = 0; i < fields.size(); i++) {
                    ...
                    // 创建列
                    cell = row.createCell(i);
                    writeHeadCell(workbook, sheet, cell, i, attr.name(), attr.prompt());
    ...
    public String encodingFilename(String filename) {
        filename = filename + "_" + TimeUuidUtil.get16UUID() + ".xls" ;
        return filename;
    }
     ...
     * @throws Exception
     */
    public String convertByExp(String propertyValue, String converterExp) throws Exception
    {
        try
        {
