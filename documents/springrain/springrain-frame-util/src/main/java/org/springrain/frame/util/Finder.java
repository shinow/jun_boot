...
public final class Finder {
    ...
    /**
     * 根据 Entity(分表)或者Class返回查询语句的Finder 等同 getSelectFinder(User.class)==return new
     * Finder("SELECT * FROM t_User ");
     *
     * @param object
     * @return
     * @throws Exception
    ...
    /**
     * 根据 Entity(分表)或者Class返回更新语句的Finder 等同
     * getUpdateFinder(User.class,"id,name")==return new Finder("SELECT id,name FROM
     * t_User ");
     *
    ...
    /**
     * 根据 Entity(分表)或者Class返回更新语句的Finder 等同 getUpdateFinder(User.class)==return new
     * Finder("UPDATE t_User SET ");
     *
     * @param object
     * @return
     * @throws Exception
    ...
    /**
     * 根据 Entity(分表)或者Class返回更新语句的Finder 等同
     * getUpdateFinder(User.class,"name=:name")==return new Finder("UPDATE t_User
     * SET name=:name ");
     *
     * @param object,updatesql
     * @return
    ...
    /**
     * 根据 Entity(分表)或者Class返回删除语句的Finder 等同 getDeleteFinder(User.class)==return new
     * Finder("DELETE FROM t_user ");
     *
     * @param object
     * @return
     * @throws Exception
    ...
    public String getSql() throws Exception {
        ...
        if (isEscapeSql() && _sql.contains("'")) {
            throw new Exception(
                    "SQL语句请不要直接拼接字符串参数!!!使用标准的占位符实现,例如  finder.append(' and id=:id').setParam('id',_id);  ");
        }
        return _sql;
    }
