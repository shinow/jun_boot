...
(function ($) {
    $.extend({
            ...
            selectDictLabel: function(datas, value) {
                ...
                $.each(datas, function(index, dict) {
                    if (dict.dictValue == ('' + value)) {
                    	...
                    	actions.push($.common.sprintf("<span class='%s'>%s%s</span>", listClass, dict.dictLabel,suffix));
                        return false;
                    }
                });
                return actions.join('');
        ...
        treeTable: {
			...
			selectRowData:function(){
				var rows = $.bttTable.bootstrapTreeTable('getSelections');
				return rows;
			},
    	...
    	modal: {
			...
			// 消息提示并刷新父窗体
			msgReloadSelf: function(msg, type) {
				layer.msg(msg, {
						icon: $.modal.icon(type),
						time: 500,
						shade: [0.1, '#8F8F8F']
            ...
            confirm: function (content, callBack) {
            	layer.confirm(content, {
        	        icon: 3,
        	        title: I18n.systip,
        	        btn: [I18n.determine, I18n.cancel]
        	    }, function (index) {
        	    	layer.close(index);
        	        callBack(true);
        	    });
            ...
            open: function (title, url, width, height, callback) {
				...
				let shadeClose = true;
				if(table.options.mastClose && table.options.mastClose.length>0){
                	try {
						$.each(table.options.mastClose,function (i,val) {
							if(url.lastIndexOf(Object.keys(val) ) != -1){
								shadeClose = Object.values(val)[0];
								return false;
							}
						})
					} catch (e) {
            ...
            openFull: function (title, url, width, height) {
    			...
    			let btn =  [I18n.determine, I18n.close];
    			if(table.options.btn == false ) {
					btn = false;
				}
    			console.log(btn);
            	//如果是移动端，就使用自适应大小弹窗
        ...
        operate: {
            ...
            detailUrl: function(id) {
            	...
            	if ($.common.isNotEmpty(id)) {
            	    ...
            	    if (id.length == 0) {
            			$.modal.alertWarning(I18n['selectOne']);
            			return;
            		}
            	    url = table.options.detailUrl.replace("{id}", id);
            	}
            ...
            remove: function(id) {
            	table.set();
            	$.modal.confirm( I18n['deleteGroup1_1'] + " " + table.options.modalName + " " + I18n['deleteGroup1_2'] , function() {
            ...
            removeAll: function() {
        		...
        		$.modal.confirm(I18n['deleteGroup2_1'] + " " + rows.length + " " + I18n['deleteGroup2_2'] , function() {
            ...
            clean: function() {
            	table.set();
            	$.modal.confirm(I18n['deleteGroup3_1'] + " "  + table.options.modalName + " " + I18n['deleteGroup3_2'] , function() {
            ...
            add: function(id) {
            	table.set();
            	$.modal.open(I18n['add'] + " " +  table.options.modalName, $.operate.addUrl(id));
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
            	table.set();
                $.modal.openTab(I18n['add'] + " " +  table.options.modalName, $.operate.addUrl(id));
            },
            // 添加信息 全屏
            addFull: function(id) {
            	table.set();
            	var url = $.common.isEmpty(id) ? table.options.createUrl : table.options.createUrl.replace("{id}", id);
                $.modal.openFull(I18n['add'] + " " +  table.options.modalName, url);
            },
            // 添加访问地址
            addUrl: function(id) {
			...
			view:function(id){
				...
				if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
					...
					$.modal.open(I18n['view'] +" "+ table.options.modalName, url);
				} else {
					$.modal.open(I18n['view'] +" "+ table.options.modalName, $.operate.viewUrl(id));
				}
			},
			// 查看 全屏
			viewFull:function(id){
				...
				$.modal.openFull(I18n['view'] +" " + table.options.modalName, url);
			},
            // 修改信息
            edit: function(id) {
            	table.set();
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    $.modal.open(I18n['update'] +" "+ table.options.modalName, url);
            	} else {
            	    $.modal.open(I18n['update'] +" "+ table.options.modalName, $.operate.editUrl(id));
            	}
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            	table.set();
            	$.modal.openTab(I18n['update'] +" "+ table.options.modalName, $.operate.editUrl(id));
            },
            // 修改信息 全屏
            editFull: function(id) {
            	...
            	$.modal.openFull(I18n['update'] +" " + table.options.modalName, url);
            },
            // 修改访问地址
            editUrl: function(id) {
            	var url = "/404.html";
        ...
        tree: {
        	...
        	init: function(options) {
        	    ...
        	    $.get(options.url, function(data) {
        			...
        			if(options.showCheckBox){
        				// console.log(tree);
						// var checkNodes = $._tree.getCheckedNodes(true) ;
						// console.log(checkNodes);
						// //tree.showNodes(checkNodes);
						// //tree.expandAll(checkNodes);  //展开所有
						//
						// for(var i = 0; i < checkNodes.length; i++){
						// 	console.log(checkNodes[i]);
						// 	tree.expandNode(checkNodes[i],true,false,true,false)
						// }
						//
						//
						// $.tree.showAllNode( tree.getCheckedNodes(true) )
					}
			...
			// 获取当前被勾选集合
			getFilterCheckedNodes: function(filterColumn,filterVal,column) {
				var _column = $.common.isEmpty(column) ? "id" : column;
				var nodes = $._tree.getCheckedNodes(true);
				return $.map(nodes, function (row) {
					console.log(row[filterColumn]);
					return row[_column];
				}).join();
			},
        	// 不允许根父节点选择
