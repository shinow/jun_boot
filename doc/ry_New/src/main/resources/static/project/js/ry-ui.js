...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            init: function(options) {
                ...
                $('#bootstrap-table').bootstrapTable({
                    ...
                    rowStyle:options.rowStyle,                           // 行的样式
            ...
            search: function(formId) {
                ...
                //每次查询将修改和删除按钮置灰
                $("#toolbar").find(".btn-edit,.btn-del").addClass("disabled");
            },
            // 下载
        ...
        treeTable: {
            ...
            search: function(formId) {
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                $._treeTable.bootstrapTreeTable('refresh', $.treeTable.getDefaultParams(currentId));
            },
            // 刷新
            refresh: function() {
        ...
        form: {
            //表单提交初始化
            init:function (options) {
                var formId = $.common.isEmpty(options.formId) ? $('form').attr('id') : options.formId;
                var _$Form = $("#"+formId);
                _$Form.attr("enctype","multipart/form-data");
                _$Form.ajaxForm({
                    url:options.url,
                    type:"post",
                    data:{},
                    ...
                    beforeSubmit:function () {
                        if($.common.isEmpty(options.beforeSubmit)){
                            $.modal.loading();
                            return true;
                        }
                        if(typeof options.beforeSubmit === "function"){
                            ...
                            $.modal.msgError("beforeSubmit必须是function对象！");
                        }
                        return false;
                    },
        ...
        modal: {
            ...
            openOptions: function (options) {
                ...
                var _width = $.common.isEmpty(options.width) ? ($(window).width()-150) : options.width;
                var _height = $.common.isEmpty(options.height) ? ($(window).height() - 50) : options.height;
                var _btn =['<i class="fa fa-check"></i> 确认', '<i class="fa fa-close"></i> 关闭'];
                var _type = $.common.isEmpty(options.type) ? 2 : options.type;
                if(options.btn){
                    _btn = options.btn;
                }
                layer.open({
        ...
        tree: {
            ...
            init: function(options) {
                ...
                $.get(options.url,options.queryParam, function(data) {
            ...
            //获取当前部门为全选且父部门为半选状态的集合
            getCheckedNodesUnderDepts:function(column){
                var _column = $.common.isEmpty(column) ? "id" : column;
                var nodes = $._tree.getCheckedNodes(true);
                return $.map(nodes, function (node) {
                    if(node.getCheckStatus().half == false){
                        if(node.getParentNode()==null ||
                            node.getParentNode().getCheckStatus().half==true){
                            return node[_column];
                        }
                    }
                }).join();
        ...
        cookies:{
            set:function (key,value,hour) {
                if($.common.isEmpty(hour)){
                    hour=1;
                }
                var exp = new Date();
                exp.setTime(exp.getTime() + hour*60*60*1000);
                var str =key + "="+ escape (value) + ";path=/;expires=" + exp.toGMTString();
                document.cookie =str;
            },
            get:function (key) {
