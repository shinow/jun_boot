...
insert into sys_config values(5, '用户管理-密码字符范围',         'sys.account.chrtype',      '0',             'Y', 'admin', sysdate, '', null, '默认任意字符范围，0任意（密码可以输入任意字符），1数字（密码只能为0-9数字），2英文字母（密码只能为a-z和A-Z字母），3字母和数字（密码必须包含字母，数字）,4字母数组和特殊字符（密码必须包含字母，数字，特殊字符-_）（目前支持的特殊字符包括：~!@#$%^&*()-=_+）');
...
create table sys_job_log (
  ...
  status              char(1)          default '0',
  exception_info      varchar2(4000)   default '',
  create_time         date
);
...
create table sys_notice (
  ...
  notice_content    varchar2(4000)  default null,
  status            char(1)         default '0',
  create_by         varchar2(64)    default '',
  create_time       date,
  update_by         varchar2(64)    default '',
