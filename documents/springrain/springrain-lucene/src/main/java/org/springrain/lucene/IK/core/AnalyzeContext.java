...
//    }
     ...
     */
    /*
     * private void compound(Lexeme result){ if(true){ return ; } //数量词合并处理
     * if(!this.results.isEmpty()){
     *
     * if(Lexeme.TYPE_ARABIC == result.getLexemeType()){ Lexeme nextLexeme =
     * this.results.peekFirst(); boolean appendOk = false; if(Lexeme.TYPE_CNUM ==
     * nextLexeme.getLexemeType()){ //合并英文数词+中文数词 appendOk =
     * result.append(nextLexeme, Lexeme.TYPE_CNUM); }else if(Lexeme.TYPE_COUNT ==
     * nextLexeme.getLexemeType()){ //合并英文数词+中文量词 appendOk =
     * result.append(nextLexeme, Lexeme.TYPE_CQUAN); } if(appendOk){ //弹出
     * this.results.pollFirst(); } }
     *
     * //可能存在第二轮合并 if(Lexeme.TYPE_CNUM == result.getLexemeType() &&
     * !this.results.isEmpty()){ Lexeme nextLexeme = this.results.peekFirst();
     * boolean appendOk = false; if(Lexeme.TYPE_COUNT ==
     * nextLexeme.getLexemeType()){ //合并中文数词+中文量词 appendOk =
     * result.append(nextLexeme, Lexeme.TYPE_CQUAN); } if(appendOk){ //弹出
     * this.results.pollFirst(); } }
     *
     * } }
