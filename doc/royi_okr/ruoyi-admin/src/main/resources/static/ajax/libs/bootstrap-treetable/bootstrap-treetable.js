    ...
    $.fn.bootstrapTreeTable = function(options, param) {
        ...
        var initToolbar = function() {
            ...
            if (options.showSearch) {
                var $searchBtn = $('<button class="btn btn-default btn-outline" type="button" aria-label="search" title="'+I18n['search']+'"><i class="glyphicon glyphicon-search"></i></button>');
                $rightToolbar.append($searchBtn);
                registerSearchBtnClickEvent($searchBtn);
            }
            // 是否显示刷新按钮
            if (options.showRefresh) {
                var $refreshBtn = $('<button class="btn btn-default btn-outline" type="button" aria-label="refresh" title="'+I18n['refresh']+'"><i class="glyphicon glyphicon-repeat"></i></button>');
                $rightToolbar.append($refreshBtn);
                registerRefreshBtnClickEvent($refreshBtn);
            }
            // 是否显示列选项
            if (options.showColumns) {
                var $columns_div = $('<div class="btn-group pull-right" title="'+I18n['column']+'"><button type="button" aria-label="columns" class="btn btn-default btn-outline dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="glyphicon glyphicon-list"></i> <span class="caret"></span></button></div>');
        ...
        var initHiddenColumns = function(){
            $.each(target.hiddenColumns, function(i, field) {
                target.find("."+field+"_tbody").hide();
                target.find("."+field+"_thead").hide();
            });
        }
        // 初始化表头
        var initHeader = function() {
            ...
            $.each(options.columns, function(i, column) {
                ...
                if (i == 0 && column.field == 'selectItem') {
                    ...
                    let field   = column.field?column.field:"default";
                    let classes = column.classes?column.classes+" ":"";
                    let align   = column.align?"text-align:"+column.align+";":"";
                    let width   = column.width?"width:"+column.width+";":"";
                    $th = $('<th style="' + align + width + '" class="'+ classes + field + '_thead"></th>');
                }
        ...
        var renderRow = function(item, isP, lv, row_id, p_id) {
            ...
            $.each(options.columns, function(index, column) {
                ...
                if (column.field == 'selectItem') {
                    ...
                    if (column.formatter) {
                        $td.append(column.formatter.call(this,item,  item[options.code], index));
                    }
                    $tr.append($td);
                } else {
                    var $td = $('<td name="' + column.field + '" class="' + column.field + '_tbody"></td>');
                    if(column.width){
                    ...
                    // 增加classes渲染
                    if (column.classes) {
                        $td.addClass(column.classes);
                    }
                    // 增加cellStyle渲染
                    if (column.style) {
                        $td.css( column.style.call(this, getItemField(item, column.field), item, index) ) ;
                    }
                    // 增加formatter渲染
                    if (column.formatter) {
        ...
        target.showColumn=function(field,flag) {
            ...
            target.find("."+field+"_tbody").show();
            target.find("."+field+"_thead").show();
            //是否更新列选项状态
