...
public class SysMenuServiceImpl implements ISysMenuService {
    ...
    public List<SysMenu> selectMenusByUser(SysUser user) {
        ...
        if (user.isAdmin()) {
            ...
            menus = menuMapper.selectMenusByUserId(user.getUserId(),"MC");
        }
     ...
     */
    @Override
    public List<Map<String, Object>> roleMenuTreeData(SysRole role,SysUser user) {
        Long roleId = role.getRoleId();
        List<Map<String, Object>> trees = new ArrayList<Map<String, Object>>();
//        List<SysMenu> menuList = menuMapper.selectMenuAll();
        //配置角色菜单时只能选择当前用户可看见的菜单
        List<SysMenu> menuList = null;
        if (user.isAdmin()) {
            menuList = menuMapper.selectMenuList(new SysMenu());
        } else {
            menuList = menuMapper.selectMenusByUserId(user.getUserId(),"MCF");
        }
